...
clockwiseOrder = [user, leftPlayer, crossPlayer, rightPlayer, user, leftPlayer, crossPlayer, rightPlayer, user, leftPlayer, crossPlayer, rightPlayer]
#this is so we can cycle through dealers and choosers, etc without index errors



def shuffleAndDeal(dealer):
  #also chooses the first dealer from the first black jsck rule
  for i in range(4):
    ...
    while True:
      ...
      if nextPlayerCardThrow != True:
        ...
        for i in cardsPlayed:
          if i[1] != user:
            print(i[1].name, "played: ", i[0].displayValue)
          else:
            break
