    ...
    if i == 0:
      ...
      for n in range(gridWidthInput - miniWidth):
        for b in range(gridHeightInput - miniHeight):
          lineageList[n * (gridHeightInput - miniHeight) + b].append(miniGrid(miniHeight, miniWidth, gridInput, n, b))
      ...
      if x != 0:
        for d in range(len(lineageList[i][x].board)):
          for f in range(len(lineageList[i][x].board[d])):
            sameChecker.update({lineageList[i][x].board[d][f].living:lineageList[i][x-1].board[d][f].living})
